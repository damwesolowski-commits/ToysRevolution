...
/// </summary>
public class MultiPlayerButtonGroupManager : MonoBehaviour
{
    private static MultiPlayerButtonGroupManager instance;
    private static Dictionary<int, GroupData> groups = new();

    private class GroupData
    {
        public List<MultiPlayerButton> buttons = new();
    ...
    // ===================================================
    public static void RegisterButton(MultiPlayerButton button)
    {
        ...
        Debug.Log($"[MultiPlayerButtonManager] ➕ Dodano przycisk GID={button.groupId}. " +
    ...
    // ===================================================
    public static void NotifyButtonPressed(int groupId, MultiPlayerButton button)
    {
        if (!groups.ContainsKey(groupId)) return;
        GroupData data = groups[groupId];

        int pressedCount = CountPressed(data);
        Debug.Log($"[MultiPlayerButtonManager] Press G{groupId}: pressed={pressedCount}/{data.requiredPlayers}");
    ...
    // ===================================================
    public static void NotifyButtonReleased(int groupId, MultiPlayerButton button)
    {
        if (!groups.ContainsKey(groupId)) return;
        GroupData data = groups[groupId];

        int pressedCount = CountPressed(data);
        Debug.Log($"[MultiPlayerButtonManager] Release G{groupId}: pressed={pressedCount}/{data.requiredPlayers}");
    ...
    {
        var registries = GameObject.FindObjectsOfType<ColorBlockRegistryBase>(true);
        if (registries.Length == 0)
        {
            Debug.LogWarning("[MultiPlayerButtonManager] ❌ Nie znaleziono żadnego rejestru bloków w scenie!");
            return;
        }
        ...
        {
            // Spróbuj przełączyć w każdym rejestrze – rejestry bez tej grupy nic nie zmienią
            r.ToggleGroup(groupId);   // używa wewnętrznie GetBlocksByGroup() i SetState() w ColorBlock
            toggledIn++;
        }

        Debug.Log($"[MultiPlayerButtonManager] ✅ Aktywowano efekt grupy {groupId} (sprawdzono {toggledIn} rejestrów)");
    }
    ...
    private static void DeactivateGroupEffect(int groupId)
    {
        ...
        Debug.Log($"[MultiPlayerButtonManager] ⛔ Dezaktywowano efekt grupy {groupId} (sprawdzono {toggledIn} rejestrów)");
    }
}
